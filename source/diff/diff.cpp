...
// A map of function names to function ids with the same name.
using IdGroupMapByName = std::unordered_map<std::string, IdGroup>;
using IdGroupMapByTypeId = std::unordered_map<uint32_t, IdGroup>;
...
class Differ {
 public:
  Differ(const opt::Module* src, const opt::Module* dst, uint32_t options)
      : src_(src),
        dst_(dst),
        options_(options),
        src_id_to_(src),
        dst_id_to_(dst),
        id_map_(src->IdBound(), dst->IdBound()) {}
  ...
  void DumpIdMap() {
    if ((options_ & OPTION_DUMP_ID_MAP) == 0) {
      return;
    }
    ...
    for (uint32_t src_id = 1; src_id < src_->IdBound(); ++src_id) {
      ...
      if (dst_id != 0)
        fprintf(stderr, "%4u -> %4u [%s]\n", src_id, dst_id,
                src_id_to_.inst_map_[src_id]
 ...
 private:
  ...
  using FunctionMap = std::unordered_map<uint32_t, const opt::Function*>;

  InstructionList GetFunctionBody(const opt::Function& function,
                                  bool include_header);
  void GetFunctionBodies(const opt::Module* module, FunctionMap* functions,
                         FunctionInstMap* function_insts);
  void GetFunctionInstructions(const opt::Module* module,
                               FunctionInstMap* function_insts);
  ...
  void GroupIds(const IdGroup& functions, bool is_src,
                std::unordered_map<T, IdGroup>* groups,
  ...
  const char* GetName(const IdInstructions& id_to, uint32_t id);
  ...
  void OutputLine(std::ostream& stream,
                  std::function<bool()> are_lines_identical,
                  std::function<void()> output_src_line,
                  std::function<void()> output_dst_line);
  template <typename InstList>
  void OutputSection(
      std::ostream& stream, const InstList& src_insts,
      const InstList& dst_insts,
  ...
  bool PrintColor() const { return (options_ & OPTION_COLOR) != 0; }
  ...
  for (uint32_t operand_index = 0; operand_index < a_operand_count;
    ...
    switch (a_operand.type) {
      ...
      case SPV_OPERAND_TYPE_LITERAL_STRING: {
        int str_compare = strcmp(a_operand.AsCString(), b_operand.AsCString());
        if (str_compare != 0) {
          return str_compare;
        }
        break;
...
bool Differ::DoesOperandMatch(const opt::Operand& src_operand,
  ...
  switch (src_operand.type) {
    ...
    case SPV_OPERAND_TYPE_LITERAL_STRING:
      return strcmp(src_operand.AsCString(), dst_operand.AsCString()) == 0;
    default:
...
bool Differ::DoInstructionsMatchFuzzy(const opt::Instruction* src_inst,
  ...
  const uint32_t in_operand_count =
      std::min(src_inst->NumInOperandWords(), dst_inst->NumInOperandWords());

  bool match = true;
  for (uint32_t in_operand_index = 0; in_operand_index < in_operand_count;
       ++in_operand_index) {
    const opt::Operand& src_operand = src_inst->GetInOperand(in_operand_index);
    const opt::Operand& dst_operand = dst_inst->GetInOperand(in_operand_index);
...
bool Differ::MatchOpTypeStruct(const opt::Instruction* src_inst,
  ...
  const char* src_name = GetName(src_id_to_, src_type_id);
  const char* dst_name = GetName(dst_id_to_, dst_type_id);
  ...
  if (src_name != nullptr && dst_name != nullptr) {
    ...
    if (strcmp(src_name, "gl_PerVertex") == 0) {
...
bool Differ::MatchOpSpecConstant(const opt::Instruction* src_inst,
  ...
  const char* src_name = GetName(src_id_to_, src_id);
  const char* dst_name = GetName(dst_id_to_, dst_id);

  // If debug info is present, always match the spec consts by name.
  if (src_name != nullptr && dst_name != nullptr) {
    return strcmp(src_name, dst_name) == 0;
  }
...
bool Differ::MatchOpVariable(const opt::Instruction* src_inst,
  ...
  // decorations.
  if ((options_ & OPTION_IGNORE_SET_BINDING) == 0) {
  ...
  // decoration.
  if ((options_ & OPTION_IGNORE_LOCATION) == 0) {
...
InstructionList Differ::GetFunctionBody(const opt::Function& function,
                                        bool include_header) {
  // Go over the instructions of the function and add the instructions to a flat
  // list to simplify future iterations.
  InstructionList body;
  bool is_function_header = true;
  function.ForEachInst(
      [&body, &is_function_header,
       include_header](const opt::Instruction* inst) {
        const SpvOp opcode = inst->opcode();
        if (is_function_header && opcode == SpvOpLabel) {
          is_function_header = false;
        }
        if (is_function_header && !include_header) {
          return;
        }
        body.push_back(inst);
      },
...
void Differ::GetFunctionBodies(const opt::Module* module,
  ...
  for (const opt::Function& function : *module) {
    ...
    InstructionList body = GetFunctionBody(function, false);
    (*function_insts)[id] = std::move(body);
  }
}

void Differ::GetFunctionInstructions(const opt::Module* module,
                                     FunctionInstMap* function_insts) {
  for (const opt::Function& function : *module) {
    InstructionList body = GetFunctionBody(function, true);
...
void Differ::GroupIds(
    const IdGroup& functions, bool is_src,
    std::unordered_map<T, IdGroup>* groups,
    std::function<T(const IdInstructions, uint32_t)> get_group) {
  assert(groups->empty());
...
void Differ::MatchIdsInFunctionBodies(const InstructionList& src_body,
  ...
  while (src_cur < src_body.size() && dst_cur < dst_body.size()) {
    if (src_match_result[src_cur] && dst_match_result[dst_cur]) {
      // Matching instructions result ids, if any.  For OpAccessChain, OpLoad
      // and OpStore instructions that reference a variable, match the variables
      // too as a best effort.
      //
      ...
      switch (src_inst->opcode()) {
        ...
        case SpvOpStore:
          ...
          if (IsVariable(src_id_to_, src_pointer_id) &&
              ...
              AreVariablesMatchable(src_pointer_id, dst_pointer_id,
                                    flexibility)) {
...
std::string Differ::GetFunctionName(const IdInstructions& id_to, uint32_t id) {
  const char* name = GetName(id_to, id);

  if (name == nullptr) {
    return "";
  }

  std::string name_without_args = name;
  return name_without_args.substr(0, name_without_args.find('('));
}
...
spv_ext_inst_type_t Differ::GetExtInstType(const IdInstructions& id_to,
  ...
  return spvExtInstImportTypeGet(set_inst->GetInOperand(0).AsCString());
}
...
void Differ::MatchExtInstImportIds() {
  ...
  // Then match the ids.
  MatchIds(potential_id_map, [this](const opt::Instruction* src_inst,
                                    const opt::Instruction* dst_inst) {
    ...
    return strcmp(src_name.AsCString(), dst_name.AsCString()) == 0;
  });
}
...
void Differ::MatchEntryPointIds() {
  ...
  for (const uint32_t execution_model : all_execution_models) {
    ...
    for (const opt::Instruction* src_inst : src_insts) {
      for (const opt::Instruction* dst_inst : dst_insts) {
        ...
        if (strcmp(src_name.AsCString(), dst_name.AsCString()) == 0) {
  ...
  // TODO: test diff with multiple entry points of different type, matching and
  // unmatching.
  // TODO: test diff with multiple entry points of same type
}
...
void Differ::MatchFunctions() {
  // First, get the body of all the functions.
  FunctionMap src_funcs;
  FunctionMap dst_funcs;
  FunctionInstMap src_func_insts;
  FunctionInstMap dst_func_insts;

  GetFunctionBodies(src_, &src_funcs, &src_func_insts);
  GetFunctionBodies(dst_, &dst_funcs, &dst_func_insts);

  IdGroup src_func_ids;
  IdGroup dst_func_ids;

  for (const auto& func : src_funcs) {
    src_func_ids.push_back(func.first);
  }
  for (const auto& func : dst_funcs) {
  ...
  for (const uint32_t src_func_id : src_func_ids) {
    ...
    // better diff.
    MatchFunctionParamIds(src_funcs[src_func_id], dst_funcs[dst_func_id]);

    // Take the diff of the two functions.
    DiffMatch src_match_result, dst_match_result;
    MatchFunctionBodies(src_func_insts[src_func_id],
                        dst_func_insts[dst_func_id], &src_match_result,
                        &dst_match_result);

    // Match ids between the two function bodies; which can also result in
    // global variables getting matched.
    MatchIdsInFunctionBodies(src_func_insts[src_func_id],
                             dst_func_insts[dst_func_id], src_match_result,
                             dst_match_result, 0);
  }
  ...
  for (const auto& src_func_group_by_type_id : src_func_groups_by_type_id) {
    ...
    BestEffortMatchFunctions(src_group_by_type_id, dst_group_by_type_id,
                             src_func_insts, dst_func_insts);
  }

  // Any function that's left, best effort match them.
  BestEffortMatchFunctions(src_func_ids, dst_func_ids, src_func_insts,
                           dst_func_insts);
}
...
void Differ::OutputLine(std::ostream& stream,
                        std::function<bool()> are_lines_identical,
                        std::function<void()> output_src_line,
                        std::function<void()> output_dst_line) {
  if (are_lines_identical()) {
    ...
    if (PrintColor()) stream << spvtools::clr::red{true};
    stream << "-";
    output_src_line();
    if (PrintColor()) stream << spvtools::clr::green{true};
    stream << "+";
    output_dst_line();
    if (PrintColor()) stream << spvtools::clr::reset{true};
  }
}
...
void Differ::OutputSection(
    std::ostream& stream, const InstList& src_insts, const InstList& dst_insts,
    std::function<void(const opt::Instruction&, const IdInstructions&,
                       const opt::Instruction&)>
        write_inst) {
...
spv_result_t Differ::Output() {
  ...
  if ((options_ & OPTION_INDENT) != 0) {
    disassembly_options |= SPV_BINARY_TO_TEXT_OPTION_INDENT;
  }

  NameMapper name_mapper = GetTrivialNameMapper();
  disassemble::InstructionDisassembler dis(grammar, stream, disassembly_options,
                                           name_mapper);

  if ((options_ & OPTION_NO_HEADER) == 0) {
    // Output the header
    ...
    OutputLine(
        stream, []() { return true; }, [&dis]() { dis.EmitHeaderSpirv(); },
        []() { assert(false && "Unreachable"); });
    OutputLine(
        stream, [this]() { return src_->version() == dst_->version(); },
        [this, &dis]() { dis.EmitHeaderVersion(src_->version()); },
        [this, &dis]() { dis.EmitHeaderVersion(dst_->version()); });
    OutputLine(
        stream, [this]() { return src_->generator() == dst_->generator(); },
    ...
    OutputLine(
        stream, [this]() { return src_->schema() == dst_->schema(); },
  ...
  auto write_inst = [this, &dis](const opt::Instruction& inst,
    ...
    // TODO: write a test where the src shader doesn't have OpExtInst, but dst
    // does. Finding the inst in the dst inst with mapped ids (in src space)
    // should work.

    dis.EmitInstruction(parsed_inst, 0);
  };

  OutputSection(stream, src_->capabilities(), dst_->capabilities(), write_inst);
  OutputSection(stream, src_->extensions(), dst_->extensions(), write_inst);
  OutputSection(stream, src_->ext_inst_imports(), dst_->ext_inst_imports(),
                write_inst);
  ...
  OutputSection(stream, src_->entry_points(), dst_->entry_points(), write_inst);
  OutputSection(stream, src_->execution_modes(), dst_->execution_modes(),
                write_inst);
  OutputSection(stream, src_->debugs1(), dst_->debugs1(), write_inst);
  OutputSection(stream, src_->debugs2(), dst_->debugs2(), write_inst);
  OutputSection(stream, src_->debugs3(), dst_->debugs3(), write_inst);
  OutputSection(stream, src_->ext_inst_debuginfo(), dst_->ext_inst_debuginfo(),
                write_inst);
  OutputSection(stream, src_->annotations(), dst_->annotations(), write_inst);
  OutputSection(stream, src_->types_values(), dst_->types_values(), write_inst);

  // Get the body of all the functions.
  FunctionInstMap src_func_insts;
  FunctionInstMap dst_func_insts;

  GetFunctionInstructions(src_, &src_func_insts);
  GetFunctionInstructions(dst_, &dst_func_insts);

  for (const std::pair<uint32_t, InstructionList>& src_func : src_func_insts) {
    const uint32_t src_func_id = src_func.first;
    const InstructionList& src_insts = src_func.second;

    const uint32_t dst_func_id = id_map_.MappedDstId(src_func_id);
    if (dst_func_id == 0) {
      OutputSection(stream, src_insts, InstructionList(), write_inst);
      continue;
    }

    const InstructionList& dst_insts = dst_func_insts[dst_func_id];
    OutputSection(stream, src_insts, dst_insts, write_inst);
  }

  for (const std::pair<uint32_t, InstructionList>& dst_func : dst_func_insts) {
    ...
    if (src_func_id == 0) {
      OutputSection(stream, InstructionList(), dst_insts, write_inst);
    }
  }
...
void Diff(const opt::Module* src, const opt::Module* dst, uint32_t options) {
  ...
  Differ differ(src, dst, options);

  // First, match instructions between the different sections of the SPIR-V.
