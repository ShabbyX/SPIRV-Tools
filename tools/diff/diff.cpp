...
int main(int argc, char** argv) {
  ...
  uint32_t options = spvtools::diff::OPTION_NONE;

  if (allow_indent) options |= spvtools::diff::OPTION_INDENT;
  if (no_header) options |= spvtools::diff::OPTION_NO_HEADER;
  if (dump_id_map) options |= spvtools::diff::OPTION_DUMP_ID_MAP;
  if (ignore_set_binding) options |= spvtools::diff::OPTION_IGNORE_SET_BINDING;
  if (ignore_location) options |= spvtools::diff::OPTION_IGNORE_LOCATION;
  ...
  if (color_is_possible && !force_no_color) {
    ...
    if (output_is_tty || force_color) {
      options |= spvtools::diff::OPTION_COLOR;
    }
  }
  ...
  spvtools::diff::Diff(src->module(), dst->module(), options);
